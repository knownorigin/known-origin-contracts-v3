...
class MerkleTree {
  constructor(elements) {
    this.elements = [...elements]; // Sort elements
    ...
    this.bufferElementPositionIndex = this.elements.reduce((memo, el, index) => {
      memo[(0, _ethereumjsUtil.bufferToHex)(el)] = index;
      return memo;
    }, {}); // Create layers
  ...
  static combinedHash(first, second) {
    ...
    return (0, _ethereumjsUtil.keccak256)(MerkleTree.sortAndConcat(first, second));
  }
  ...
  getProof(el) {
    let idx = this.bufferElementPositionIndex[(0, _ethereumjsUtil.bufferToHex)(el)];
