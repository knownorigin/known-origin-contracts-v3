...
contract('KODAV3Marketplace reserve auction tests', function (accounts) {
  const [owner, minter, koCommission, contract, bidder1, bidder2, proxy, anotherAddress] = accounts;
  ...
  describe('all tests', () => {
    ...
    describe('End to end reserve auctions - placeBidOnReserveAuctionFor()', () => {
      ...
      it('Successfully results a full reserve auction with no start date', async () => {
        ...
        // place a bid as bidder 1
        await this.marketplace.placeBidOnReserveAuctionFor(editionId, bidder1, {
          from: anotherAddress,
          value: ether('1')
        });
    ...
    describe('withdrawBidFromReserveAuction()', () => {
      ...
      it('Can withdraw a bid before the reserve is met', async () => {
        ...
        const {receipt} = await this.marketplace.withdrawBidFromReserveAuction(EDITION_ONE_ID, {
          from: bidder1,
          gasPrice
        });
      ...
      describe('when bid placed', () => {
        ...
        describe('As bidder', () => {
          it('Can emergency exit if approval removed', async () => {
            ...
            const gasPrice = new BN(web3.utils.toWei('1', 'gwei').toString());
            const tx = await this.marketplace.emergencyExitBidFromReserveAuction(EDITION_ONE_ID, {
              from: bidder1,
              gasPrice
            });
