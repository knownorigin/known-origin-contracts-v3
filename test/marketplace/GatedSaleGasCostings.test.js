...
contract('BasicGatedSale complex tests...', function () {
  ...
  // describe.only('Mod 1000', async () => {
  //   it('gas costs', async () => {
  //     const mintSize = 500;
  //     for (let i = 0; i < mintSize; i++) {
  //       console.log(`Token ID ${i} - ${i % (parseInt(mintSize / 2))}`);
  //     }
  //     expect(true).to.be.true;
  //   });
  // });

  describe.only('Buying all 500 editions', async () => {
    it('gas costs', async () => {
      for (let i = 0; i < 500; i++) {
        console.log(`Minting ${i}`);
        await this.basicGatedSale.connect(buyer1).mint(
