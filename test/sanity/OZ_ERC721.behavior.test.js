...
const firstTokenId = '11000';
const secondTokenId = '12000';
const nonExistentTokenId = '99999999999';
const baseURI = 'https://api.com/v1/';

const RECEIVER_MAGIC_VALUE = '0x150b7a02';

let accounts, ERC721ReceiverMock;
let owner, minter, contract, approved, anotherApproved, operator, other;
let ownerAsSigner, minterAsSigner, contractAsSigner,
    approvedAsSigner, anotherApprovedAsSigner, operatorAsSigner,
    otherAsSigner;


const getAccount = address => accounts.find(account => account.address === address);

const shouldBehaveLikeERC721 = (errorPrefix) => {

    // FIXME contract method .estimateGas and .abi don't work because it's a Signer not a contract
/*
...
*/
    ...
    beforeEach(async function() {
        ...
        [owner, minter, contract, approved, anotherApproved, operator, other] = addresses;

        // Token contract as called from different accounts
        ownerAsSigner = this.token.connect(getAccount(owner));
        minterAsSigner = this.token.connect(getAccount(minter));
        contractAsSigner = this.token.connect(getAccount(contract));
        approvedAsSigner = this.token.connect(getAccount(approved));
        anotherApprovedAsSigner = this.token.connect(getAccount(anotherApproved));
        operatorAsSigner = this.token.connect(getAccount(operator));
        otherAsSigner = this.token.connect(getAccount(other));
    ...
    context('with minted tokens', function() {
        ...
        beforeEach(async function() {
            await contractAsSigner.mintToken(owner, baseURI);
            await contractAsSigner.mintToken(owner, baseURI);
        ...
        describe('balanceOf', function() {
            ...
            context('when the given address does not own any tokens', function() {
                it('returns 0', async function() {
                    expect(await this.token.balanceOf(other))
                        .equal('0');
                });
            });
            ...
            context('when querying the zero address', function() {
                it('throws', async function() {
                    expect(this.token.balanceOf(ZERO_ADDRESS))
                        .revertedWith('ERC721_ZERO_OWNER');
                });
            });
        });
        ...
        describe('ownerOf', function() {
            ...
            context('when the given token ID was not tracked by this token', function() {
                ...
                it('reverts', async function() {
                    expect(this.token.ownerOf(tokenId))
                        .revertedWith('ERC721_ZERO_OWNER');
                });
            });
        });
...
/*
        // FIXME setting 'from:' on contract methods passed into tests won't work
        describe('transfers', function() {
            ...
            beforeEach(async function() {
                const ownerAsSigner = this.token.connect(getAccount(owner));
                await ownerAsSigner.approve(approved, tokenId);
                await ownerAsSigner.setApprovalForAll(operator, true);
            });
            ...
            const transferWasSuccessful = function ({ owner, tokenId, approved }) {
                ...
                it('adjusts owners balances', async function() {
                    expect(await this.token.balanceOf(owner)).to.be.equal('1');
                });

                it('adjusts owners tokens by index', async function() {
                    if (!this.token.tokenOfOwnerByIndex) return;

                    expect(await this.token.tokenOfOwnerByIndex(this.toWhom, 0)).to.be.equal(tokenId);

                    expect(await this.token.tokenOfOwnerByIndex(owner, 0)).to.be.not.equal(tokenId);
                });
            };
            ...
            const shouldTransferTokensByUsers = function (transferFunction) {
                ...
                context('when sent to the owner', function() {
                    ...
                    it('keeps the owner balance', async function() {
                        expect(await this.token.balanceOf(owner)).to.be.equal('2');
                    });
                ...
                const shouldTransferSafely = function (transferFun, data) {
                    ...
                    describe('to a valid receiver contract', function() {
                        ...
                        it('calls onERC721Received', async function() {
                            ...
                            await expectEvent.inTransaction(receipt.tx, ERC721ReceiverMock.address, 'Received', {
                                operator: owner,
                                from: owner,
                                tokenId: tokenId,
                                data: data,
        ...
        // FIXME use the mintToken method?
...
*/
        ...
        // WIP: Working to get these cleared
        describe('approve', function() {
            ...
            const itClearsApproval = function() {
                it('clears approval for the token', async function() {
                    expect(await this.token.getApproved(tokenId))
                        .equal(ZERO_ADDRESS);
                });
            };
            ...
            context('when clearing approval', function() {
                context('when there was no prior approval', async function() {
                    await ownerAsSigner.approve(ZERO_ADDRESS, tokenId);
                    console.log(ZERO_ADDRESS)
                    itClearsApproval();
                    // FIXME agree that we don't care about 0 address approval events
                    //itEmitsApprovalEvent(ZERO_ADDRESS);
                });

                context('when there was a prior approval', async function() {
                        await ownerAsSigner.approve(approved, tokenId);
                        await ownerAsSigner.approve(ZERO_ADDRESS, tokenId);
...
function shouldBehaveLikeERC721Metadata (errorPrefix, name, symbol) {
    ...
    beforeEach(async function() {
        accounts = await ethers.getSigners();
        const addresses = accounts.map(a => a.address);
        [owner, minter, contract] = addresses;
    })
